            	...
            	<div class="general_content">
                	<div class="main_content">
                        ...
                        <article class="block_single_news">
                          ...
                          <p class="title"><a>${requestScope.newsDetail.title }</a></p>
                           ...
                           <!--  <p class="subtitle">Many variations of passages of available, but the majority have suffered alteration in some form. Humour, or randomised words which don't look even slightly believable.</p> -->
                            ...
                            <div class="info">
                            	...
                            	<div class="r_part">
                                	<div class="category"><p><a href="#">${requestScope.newsDetail.typeName }</a></p></div>
												 ...
												 <c:if  test="${sessionScope.loginUser.power==null }"><!-- 判断是不是会员：==3是会员 -->
														   <c:if test="${comment.status==0 }"><!-- 权限是0，表示允许发言 -->
																...
																<div class="content">
																	...
																	<p class="name">用户ID：${comment.userId }</p> 
																	<p class="info">评论时间：${comment.writeTime }</p>
																</div>
													 ...
													 <c:if  test="${sessionScope.loginUser.power==3 }"><!-- 判断是不是会员：==3是会员 -->
														   <c:if test="${comment.status==0 }"><!-- 权限是0，表示允许发言 -->
																...
																<tr>
																	...
																	<td nowrap><div class="author"><p>用户ID：${comment.userId }</p></div></td>
																	<td>
																		<div class="date">
																			<p>评论时间：${comment.writeTime }</p>
																			<%-- <a href="CommentServlet?op=delete&id=${comment.id }&newsId=${requestScope.newsDetail.id }">删除</a> --%>
																		</div>
																	</td>
																</tr>
				 											  <%-- 	${comment.content }
				 											  	<div class="name"><p><a href="#">发布者ID：${comment.userId }</a></p></div>
				 											
				 												<div class="date"><p>发布时间：${comment.writeTime }</p></div>
																			发布时间：${comment.writeTime }
																		<a href="CommentServlet?op=delete&id=${comment.id }&newsId=${requestScope.newsDetail.id }">删除</a> --%>
														   ...
														   <c:if  test="${sessionScope.loginUser.power==1 }"><!-- 如果身份==1，即管理员 -->
			  												   <tr> 
			  													...
			  													<td><span>发布者ID：${comment.userId }</span></td>
																<td>		发布时间：${comment.writeTime }
																		<a href="commentservlet?op=delete&id=${comment.id }&newsId=${requestScope.newsDetail.id }">删除</a>
																	<%-- <c:if test="${comment.status==0 }"><a href="commentservlet?op=changStatus&id=${comment.id }&newsId=${requestScope.newsDetail.id }">隐藏评论</a></c:if>
																	<c:if test="${comment.status==1 }"><a href="commentservlet?op=changStatus&id=${comment.id }&newsId=${requestScope.newsDetail.id }">显示评论</a></c:if>
																	 --%>
																</td>
																</tr>
			  											</c:if>
