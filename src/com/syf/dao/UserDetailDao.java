...
public class UserDetailDao {
		...
		try {
			while (rs.next()) {
				UserDetail user = new UserDetail(rs.getInt("id"),
						...
						rs.getString("phone"), rs.getInt("power"),
						rs.getInt("status"), rs.getDate("registerTime"));
				list.add(user);
			}
	...
	public int save(UserDetail userDetail) {
		String sql = "insert into userDetail "
				+ "(userName,userPwd,phone,power,status,registerTime)"
				+ "values(?,?,?,?,?,?)";
		Object[] params = { userDetail.getUserName(), userDetail.getUserPwd(),
				userDetail.getPhone(), userDetail.getPower(),
				userDetail.getStatus(), userDetail.getRegisterTime() };
		return dbUtil.executeUpdate(sql, params);
	}

	// 更新用户数据到数据库
	public int update(UserDetail userDetail) {
		String sql = "update userDetail set userName=?,userPwd=?,phone=?,power=?,status=?,registerTime=?,"
				+ "where id=?";
		Object[] params = { userDetail.getUserName(), userDetail.getUserPwd(),
				userDetail.getPhone(), userDetail.getPower(),
				userDetail.getStatus(), userDetail.getRegisterTime(),
	...
	public void changeStatus(int id) {
		String sql = "update userDetail set status=(status+1)%2 where id=?";
		Object[] params = { id };
	...
	public List<UserDetail> search(UserDetail userDetail) {
		...
		if (userDetail.getPower() != -1) {
			buffer.append(" and power=?");
			params.add(userDetail.getPower());
		}
		...
		if (userDetail.getStartTime() != null
				&& userDetail.getEndTime() != null) {
			buffer.append(" and registerTime between ? and ?");
			params.add(userDetail.getStartTime());
			params.add(userDetail.getEndTime());
		} else if (userDetail.getStartTime() != null) {
			buffer.append(" and registerTime >?");
			params.add(userDetail.getStartTime());
		} else if (userDetail.getEndTime() != null) {
			buffer.append(" and registerTime <?");
			params.add(userDetail.getEndTime());
		}
